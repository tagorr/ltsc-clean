...
try {
    ...
    $armed = ($null -ne $wl -and
              ...
              [int]($wl.AutoLogonCount | ForEach-Object { [CmdletBinding()]
param()
...
# Fast idempotency check: autologon already armed?
try {
    $wl = Get-ItemProperty -Path $winlogonPath -ErrorAction SilentlyContinue
    if ($null -ne $wl -and
        $wl.DefaultUserName   -eq 'bootstrap' -and
        $wl.DefaultDomainName -eq $env:COMPUTERNAME -and
        [int]($wl.AutoLogonCount | ForEach-Object { $_ }) -gt 0) {
        ...
        Write-SetupLog "[INFO] Bootstrap autologon already armed; RunOnce verified"
        return
    }
} catch { }
...
} catch { Write-SetupLog "[INFO] Autologon hardening failed" }# 2.6) Autologon hardening (remove blockers, ignore Shift)
try {
  & reg.exe DELETE "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v LegalNoticeCaption /f | Out-Null
  & reg.exe DELETE "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v LegalNoticeText    /f | Out-Null
  & reg.exe ADD    "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v DontDisplayLastUserName /t REG_DWORD /d 0 /f | Out-Null
  & reg.exe ADD    "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon"     /v IgnoreShiftOverride     /t REG_SZ    /d 1 /f | Out-Null
} catch { Write-SetupLog "[INFO] Autologon hardening failed" }# 3) Arm one-time autologon
try {
    ...
    New-ItemProperty -Path $winlogonPath \2                  -PropertyType DWord  -Force | Out-Null
...
Write-SetupLog "[INFO] RunOnce registered: CreatePrimaryAdmin.ps1" }) -gt 0)
