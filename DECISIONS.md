...
4. Post-install configuration completes. `SetupComplete.cmd` signals the `Panther flag` via `:flag_reboot` (write + verify) when servicing returned `3010/1641` or when `ALWAYS_REBOOT_AFTER_FIRST_LOGON=1` is set; the marker is written as a single-line ASCII (7-bit) token with CRLF (no BOM) and verification reads the first line only (PowerShell/.NET writer; process-boundary TYPE capture + first-line-only parsing). I/O failure classification is not based on `%ERRORLEVEL%` from CMD < / > redirections. Terminal verify-time failures abort retries immediately (no FAIL→OK logs in a single run). If a pre-existing flag is detected, it is preserved as a sticky pending reboot marker. In all other cases the flag is not created and no reboot is requested. Signaling is observable via `REBOOT_FLAG_SIGNAL_BEGIN` / `REBOOT_FLAG_SIGNAL_OK` / `REBOOT_FLAG_SIGNAL_FAIL`; failures fail closed (`FAILED=1`, tracked RC `9001`).
