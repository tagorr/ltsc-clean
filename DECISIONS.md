...
**Motivation.** Predictable phases, a clear logging model, no races during OOBE, and idempotent behavior. `SetupComplete.cmd` never performs a reboot. Instead, it decides whether a post install reboot is required and signals that decision via `%WINDIR%\Panther\_needs_reboot.flag` (the "Panther flag"), leaving consumption of that flag to Stage B of `CreatePrimaryAdmin.ps1` after the first interactive logon. The marker may be signaled early when the reboot requirement is discovered and confirmed again later; repeated signaling is intentional resilience and does not change the reboot decision model.
...
2. After OOBE completes, Windows executes `SetupComplete.cmd` as SYSTEM. In addition to servicing and hardening, `SetupComplete.cmd`:
   ...
   * only when both secrets are present and valid and the script has not failed, registers the `\L2C\CreatePrimaryAdmin` executor task first and then primes AutoAdminLogon for `bootstrap`. After Winlogon priming it attempts to write the non-secret arming marker `HKLM\SOFTWARE\L2C\AutologonPrimed (REG_DWORD)=1`. If any priming sub-step fails after task creation it rolls back Winlogon autologon-related values and attempts best-effort deletion of the task; if Winlogon priming succeeded but the marker write fails, it rolls back Winlogon autologon, keeps the scheduled task for manual continuation, and rolls back the temporary logon tweaks to safe values; Stage A reads `.primaryadmin.pw` directly under SYSTEM;
...
4. Post-install configuration completes. `SetupComplete.cmd` signals the `Panther flag` via `:flag_reboot` (write + verify) when a reboot requirement is discovered (for example when a servicing RC returns `3010/1641`) and may confirm it again in the final reboot-flag evaluation block; the marker is written as a single-line ASCII (7-bit) token with CRLF (no BOM) and verification reads the first line only (PowerShell/.NET writer; process-boundary TYPE capture + first-line-only parsing). I/O failure classification is not based on `%ERRORLEVEL%` from CMD < / > redirections. Terminal verify-time failures abort retries immediately (no FAIL→OK logs in a single run). If a pre-existing flag is detected, it is preserved as a sticky pending reboot marker. In all other cases the flag is not created and no reboot is requested. Signaling is observable via `REBOOT_FLAG_SIGNAL_BEGIN` / `REBOOT_FLAG_SIGNAL_OK` / `REBOOT_FLAG_SIGNAL_FAIL`; failures fail closed (`FAILED=1`, tracked RC `9001`).
...
- Security: `.bootstrap.pw` is created at runtime by `BootstrapLocalAdmin.ps1` using an atomic create-with-DACL model (CreateNew + FileSecurity) and verified fail-closed (SYSTEM + local Administrators only, inheritance disabled; Hidden+System). `.primaryadmin.pw` is operator/offline provisioned but must match the same required ACL/attribute shape and content constraints; it is verified fail-closed by `ValidateSecrets.ps1` and the `SetupComplete.cmd` gate. On the normal path Stage B deletes them after a successful run; in recovery they are preserved for another attempt and should be removed manually once no longer needed.
...
- Maintain two secrets under `%WINDIR%\Setup\Scripts`: `.bootstrap.pw` (generated by `BootstrapLocalAdmin.ps1` for the temporary `bootstrap` account) and `.primaryadmin.pw` (operator-supplied for `primaryadmin`). Both are single-line UTF-8 (no BOM) files with inheritance disabled (protected DACL; no inherited ACEs), Allow-only ACEs for `NT AUTHORITY\SYSTEM` (S-1-5-18) and the local Administrators group (S-1-5-32-544) only, and effective rights for each required SID include FullControl (multiple/duplicate Allow ACEs allowed), plus Hidden + System attributes.
...
- `SetupComplete.cmd` requires `.bootstrap.pw` to exist and be non-empty and contain only allowed characters on its first line, and `.primaryadmin.pw` to exist, pass the ACL/attribute check, and be readable with the allowed character set. It enforces an ACL-before-read contract for `.bootstrap.pw`: the first line is not read until SEC-2 confirms ACL/attributes OK (`L2C_BOOTSTRAP_PW_ACL_OK==1`). A single gate (FAILED=0 plus both secrets validated and loaded) controls temporary logon policy relaxation, Winlogon priming for `bootstrap` (`DefaultUserName`/`DefaultDomainName`/`DefaultPassword` read from the first line of `.bootstrap.pw`), and registration of the SYSTEM/Highest OnLogon task `\L2C\CreatePrimaryAdmin` without embedding passwords.
...
- If the gate fails, `SetupComplete.cmd` logs the reason, skips autologon and task creation, and exits in SetupComplete recovery (non-zero RC) rather than leaving partial state.
