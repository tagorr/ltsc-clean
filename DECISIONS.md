...
- Stage B of `CreatePrimaryAdmin.ps1` now:
  ...
  - Verifies post-action that Winlogon is sanitized (treats read errors as failure); if verification fails, Stage B hard-fails and refuses to teardown the `bootstrap`/`\L2C\CreatePrimaryAdmin` executor path and suppresses automatic reboot.
