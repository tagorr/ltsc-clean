...
* **SetupComplete.cmd:** servicing/logging plus secret validation and the combined continuation-gate state after DISM feature servicing, followed by final reboot-obligation selection and signaling, then the Stage B gateway (optional Stage B scheduling/Winlogon priming when the combined gate is open). When a reboot is required, servicing-driven need preserves `need-reboot`, while `ALWAYS_REBOOT_AFTER_FIRST_LOGON=1` selects `force-reboot` and sets `NEEDS_REBOOT=1`; the final Panther flag is written and positively verified before Stage B registration and Winlogon autologon priming are allowed to proceed. Signaling failure keeps those later continuation actions closed. It never calls `shutdown.exe`.
