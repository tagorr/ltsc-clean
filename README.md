...
4. After OOBE completes, Windows runs `SetupComplete.cmd` as SYSTEM exactly once. The script:
   ...
   * calls `ValidateSecrets.ps1` after DISM servicing sections and later, before the reboot-flag evaluation to check ACL/attribute shape for `.bootstrap.pw` and `.primaryadmin.pw` without reading passwords; the validator runs with `Set-StrictMode -Version Latest` and `$ErrorActionPreference='Stop'` and returns a 0-3 exit-code bitmask (0=both invalid, 1=bootstrap only, 2=primary only, 3=both valid) when it completes successfully, or `4` on an internal error. `SetupComplete.cmd` decodes `0-3` from `%ERRORLEVEL%` into `L2C_BOOTSTRAP_PW_ACL_OK` and `L2C_PRIMARYADMIN_PW_ACL_OK`, logs `[SECTION] Secret ACL validation (bootstrap=..., primaryadmin=...)`, and uses these flags for the gate; when the validator returns `4` it logs the internal failure, sets `FAILED=1`, keeps both ACL flags at `0`, and stays in the fail-closed recovery path (no Stage B registration); any other validator exit code is treated as unexpected/out-of-contract and fatal: in the secrets gate unexpected-RC branch, `SetupComplete.cmd` logs an `[ERROR]` with the rc, calls `:track_rc_secrets` (captures the first out-of-contract rc verbatim, including `3010`/`1641`) so `FINAL_RC` can reflect it when it is the first bad rc, sets `FAILED=1`, and keeps both ACL flags at `0`.
    * logs `.primaryadmin.pw` status in `SetupComplete.log` when the secrets validation block is reached (after the platform gate passes): present/missing; SEC-2 ACL/attributes `OK`/`BAD`, or `skipped (file missing)`; when `FAILED=1` it does not read `.primaryadmin.pw` contents and logs that the content load was skipped due to earlier failure;
    * reads `%WINDIR%\Setup\Scripts\.primaryadmin.pw` via `set /p` (first line only) only after SEC-2 confirmed `.primaryadmin.pw` ACL/attributes OK (`L2C_PRIMARYADMIN_PW_ACL_OK==1`), and does not read `.bootstrap.pw` first line until bootstrap SEC-2 confirmed ACL/attributes OK (`L2C_BOOTSTRAP_PW_ACL_OK==1`); validates the secrets (allowed characters only, must be non-empty as read), and only if `.bootstrap.pw` exists and is non-empty, the primary admin secret is valid, ACL flags are OK, and `FAILED=0`:
   ...
   * signals reboot requirement via `:flag_reboot` (write + verify) by creating `%WINDIR%\Panther\_needs_reboot.flag` only when a reboot is required (`NEEDS_REBOOT=1`) or `ALWAYS_REBOOT_AFTER_FIRST_LOGON=1` is set; signaling may occur early when the reboot requirement is first discovered (for resilience against premature interruption) and may be confirmed again in the final reboot-evaluation block, without changing the reboot decision model. The marker is a single-line ASCII (7-bit) token (`need-reboot` by default; `force-reboot` when `ALWAYS_REBOOT_AFTER_FIRST_LOGON=1`) written with CRLF (no BOM) and verified by reading the first line only (PowerShell/.NET writer; process-boundary TYPE capture + first-line-only parsing). I/O failure classification is not based on `%ERRORLEVEL%` from CMD < / > redirections. Terminal verify-time failures abort retries immediately (no FAIL→OK logs in a single run). If the flag already exists at `SetupComplete.cmd` start, the script logs a WARN and preserves it as a sticky pending reboot marker (it is not deleted on entry); the script does not scan prior `SetupComplete.log` history. If signaling cannot be verified, SetupComplete fails closed (`FAILED=1`, tracked RC `9001`).
   * aggregates a final exit code (`FINAL_RC`) from these signals and emits a final outcome marker immediately before logging "[RC] returning %FINAL_RC%": `[FINAL] FAIL (FINAL_RC=...)`, `[FINAL] DEGRADED manual_login_required=1`, `[FINAL] SUCCESS`, or `[FINAL] SUCCESS_WITH_HARDENING_WARNINGS`;
   * when best-effort hardening warnings were collected, attempts to deduplicate them, surfaces `warn_count_unique=<n>`, and after the `[RC] returning ...` line emits `[HARDENING] warn_count_unique=<n> file=<temp-artifact-path>` followed by the warning lines from the warning file;
...
In that case, SetupComplete may write a standalone `%ProgramData%\l2c_triage_<timestamp>_<unique>.log` entry with a single `[timestamp] WARN_REBOOT_FLAG_NO_EXECUTOR ...` line for centralized triage.
SetupComplete.log also includes `triage_log=<path>`.
...
Immediately before the tail `[RC] returning ...` line, `SetupComplete.cmd` emits a final outcome marker: `[FINAL] SUCCESS` or `[FINAL] SUCCESS_WITH_HARDENING_WARNINGS` (both exit code `0`), `[FINAL] DEGRADED manual_login_required=1` (exit code `2`), or `[FINAL] FAIL (FINAL_RC=...)` (exit code `FINAL_RC`). Hard failures take precedence over DEGRADED and are not overridden to `2`. Best-effort hardening warnings do not by themselves change `FINAL_RC` away from `0`.

When hardening warnings are present, `:hardwarn_finalize` attempts to deduplicate the collected one-line warnings and finalizes `warn_count_unique`, and `:hardwarn_emit` logs `[HARDENING] warn_count_unique=<n> file=<artifact>` after the `[RC] returning ...` line, appends the warning lines from the warning file to `SetupComplete.log`, and best-effort deletes the warning file afterward. The artifact path is surfaced in the log and is normally `%ProgramData%\l2c_hardening_warnings_<ts>_<guid>.log`, with fallback to `%SystemRoot%\Temp\l2c_hardening_warnings_<ts>_<guid>.log` when needed.
...
Regardless of failure source (platform gate, DISM, or other checks), `SetupComplete.cmd` aggregates a final RC (`L2C_FIRST_BAD_RC` if present, else `1` when `FAILED==1`, else `2` when `L2C_AUTOLOGON_DEGRADED==1`, else `0`), emits the matching `[FINAL] ...` marker, logs `[RC] returning %FINAL_RC%`, and exits with that code. When hardening warnings exist, the `[HARDENING] warn_count_unique=... file=...` summary block follows without changing that RC by itself.
...
3. `SetupComplete.cmd` (runs once after OOBE as SYSTEM) performs:
   ...
   * signals reboot requirement via `%WINDIR%\Panther\_needs_reboot.flag` only when a reboot is required (`NEEDS_REBOOT=1`, for example after `3010`/`1641`) or `ALWAYS_REBOOT_AFTER_FIRST_LOGON=1` is set; signaling may occur early when the reboot requirement is first discovered and may be confirmed again in the final reboot-evaluation block;
