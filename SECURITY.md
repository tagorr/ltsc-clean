...
* `ValidateSecrets.ps1` can encounter internal failures (for example unexpected I/O errors or unhandled exceptions in the validation logic). In those cases it logs diagnostics and exits with code `0`.
* `SetupComplete.cmd` treats exit `0` the same as “both secrets failed validation”: it decodes `bootstrap=0, primaryadmin=0`, keeps the gate closed, and does not register Stage B.
* This behavior is intentional to preserve a simple, fail-closed contract between the validator and SetupComplete; operators cannot distinguish “both secrets invalid” from “validator crashed” using the exit code alone.
* When the gate shows `bootstrap=0, primaryadmin=0`, treat the secrets as unusable and use the available logs to determine whether misconfigured secret files or a validator/environment failure is the root cause.
