...
* Best-effort hardening verification warnings remain non-fatal by themselves but are not silent: when the run stays at `FINAL_RC=0` and is not degraded, `SetupComplete.cmd` switches the final marker from `[FINAL] SUCCESS` to `[FINAL] SUCCESS_WITH_HARDENING_WARNINGS`, then emits `[HARDENING] warn_count_unique=<n> file=<artifact>` and the warning lines from the warning file. This transparency does not itself force a non-zero exit code.
...
`PreOOBE.cmd` writes `%WINDIR%\Panther\PreOOBE.log` and redirects stdout/stderr from `BootstrapLocalAdmin.ps1` into the same file; `BootstrapLocalAdmin.ps1` emits structured `[BOOTSTRAP] [INFO|WARN|ERROR] ...` lines for bootstrap lifecycle steps without logging the password or derived secrets, so the log is safe to review for bootstrap failures. PreOOBE also emits phase-scoped `[SECTION]` markers and a tail outcome (`----- PreOOBE finished -----`, `[FINAL] ...`, `[RC] returning 0|1`) to avoid interpreting policy-phase completion as overall success.
...
* `SetupComplete.cmd` enforces a single gate for temporary logon policy relaxations, Winlogon autologon, and registration of `\L2C\CreatePrimaryAdmin`: `FAILED=0`, `.bootstrap.pw` exists and is non-empty, `.primaryadmin.pw` is present, SEC-2 confirms ACL/attributes OK for both secrets (`L2C_BOOTSTRAP_PW_ACL_OK=1`, `L2C_PRIMARYADMIN_PW_ACL_OK=1`), and `.primaryadmin.pw` is read/validated successfully. If any condition fails (including either bit being `0` or `RC=4` from the validator), it logs why, skips autologon and Stage B registration, keeps only safe policy changes, and exits in SetupComplete recovery with a non-zero RC.
* `.bootstrap.pw` first-line content is not read until SEC-2 confirmed ACL/attributes OK (`L2C_BOOTSTRAP_PW_ACL_OK==1`); it is then validated against the allowed character set at the `SetupComplete.cmd` gate. Invalid characters fail closed (`FAILED=1`), skip autologon/Stage B registration, and emit an explicit `[ERROR]` without logging secret contents.
