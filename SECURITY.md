...
In the normal path, verified restore allows teardown to proceed, but normal success still requires both bootstrap disable and scheduled-task deletion to succeed. If restore verification fails, executor teardown is incomplete, or the run remains in recovery posture, the run is classified non-success and the temporary executor state and secrets may be preserved for investigation or retry.
