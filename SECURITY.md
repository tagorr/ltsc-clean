...
* At the start of `SetupComplete.cmd`, `ValidateSecrets.ps1` checks that both secret files are leaf files, have inheritance disabled, carry only the `SYSTEM` and Administrators FullControl ACEs, and include Hidden+System attributes. It never reads the passwords and returns only two flags: `L2C_BOOTSTRAP_PW_ACL_OK` and `L2C_PRIMARYADMIN_PW_ACL_OK`.
* `SetupComplete.cmd` enforces a single gate for temporary logon policy relaxations, Winlogon autologon, and registration of `\L2C\CreatePrimaryAdmin`: `FAILED=0`, `.bootstrap.pw` exists and is non-empty, `.primaryadmin.pw` is present, passes the ACL/attribute check, is read successfully, and both ACL flags are `1`. If any condition fails, it logs why, skips autologon and Stage B registration, keeps only safe policy changes, and exits in recovery with a non-zero RC.
