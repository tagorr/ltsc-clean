...
The dedicated `SNMP.Client~~~~0.0.1.0` and `WMI-SNMP-Provider.Client~~~~0.0.1.0` targets are best-effort hardening reductions, not a security boundary or mandatory completion invariant. `SetupComplete.cmd` attempts removal only when the captured capability state is parsed as `Installed` or `Staged`; probe uncertainty, nonzero probe results, missing or unparseable state, `Unknown`, `NotApplicable`, and other unsupported states may warn, skip, and continue. An actual `/Remove-Capability` mutation still follows the normal DISM policy, so an unexpected mutation failure remains fatal. Post-removal verification is non-gating, and successful pipeline completion does not prove that both capabilities are absent.
