...
* The temporary `bootstrap` administrator password is set during the early PreOOBE phase using native operating system tooling. It may be observable in short-lived process command lines or low-level logs inside this early phase. If ACL hardening of `.bootstrap.pw` fails in `BootstrapLocalAdmin.ps1`, the script attempts to delete the secret and logs success or failure so it is not intentionally left behind with incorrect ACLs (any deletion failure is explicitly visible in `PreOOBE.log`); `SetupComplete`/`ValidateSecrets` treat a missing or empty `.bootstrap.pw` as `bootstrap=0`, the gate stays closed, and Stage B is not registered. This is an accepted, limited risk for a short-lived account that is disabled and cleaned up by the end of the pipeline.
